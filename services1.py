...
class TradingEngine:
    ...
    def __init__(self, symbol: str, infra: IMarketDataSource, deribit_infra=None, repository=None):
        ...
        self.spoofing_analyzer = SpoofingAnalyzer(config)  # FIX: GEMINI - требует config для мульти-ассет
    ...
    async def _consume_and_analyze(self):
        ...
        while True:
            ...
            # 4. FIX VULNERABILITY #3: Получаем ТОЛЬКО события старше delay_sec
            # WHY: Time-window filtering гарантирует что Trade и Depth обрабатываются ВМЕСТЕ
            # - Trade T=0ms добавлен в buffer
            # - Depth T+30ms добавлен в buffer
            # - pop_ready() возвращает ОБА события (если delay=50ms прошло)
            sorted_events = self.buffer.pop_ready()  # Time-window filtering
            ...
            for event in sorted_events:
                ...
                # --- ВАРИАНТ Б: Сделка (TradeEvent) ---
                    ...
                    # === FIX VULNERABILITY #4: Check warm-up after price update ===
                    # WHY: Автоматически устанавливаем is_warmed_up при готовности
                    self.feature_collector.check_warmup()
